...
install -d -o root -g dtu -m 0750 "$APP_ROOT" "$RELEASES_DIR" "$RELEASE_DIR"
...
if [[ ! -f /etc/dtu-control.env ]]; then
  ...
  echo "Created /etc/dtu-control.env. Configure passwords, HTTPS URLs, SMTP and backup values before launch."
fi
...
echo "AI PC release $RELEASE_ID installed without copying local secrets, runtime data, logs, Git history, or development dependencies."
